...
| 0.2 | Palet warna | Deep Navy + Electric Blue, aksen hijau/oranye, merah hanya kritis | Merah/hitam/putih (Rosso Corsa `#d40000`) di `app/globals.css`; `MASTER.md` justru ungu+hijau | Tiga sumber warna berbeda, tidak ada yang menang | **DIPUTUSKAN 2026-08-09: merah/hitam/putih (Rosso Corsa `#d40000`) menang.** Aturan dokumen "Red only for critical warnings" **dicabut**; merah dipakai untuk primary *dan* destructive. Konsekuensi yang diterima: hot lead tidak punya oranye khusus dan peringatan kritis tidak lagi dibedakan lewat hue — keduanya **wajib** dibedakan lewat ikon + label, tidak boleh warna saja. `MASTER.md` (ungu+hijau) resmi mati sebagai acuan warna |
| 0.2b | Font | Inter / Plus Jakarta Sans / Manrope | Plus Jakarta Sans + Inter | Heading & body seluruh aplikasi | **DIPUTUSKAN 2026-08-09: ikut dokumen** — Plus Jakarta Sans (heading) + Inter (body), sebelumnya Lexend + Source Sans 3. Diterapkan di `app/layout.tsx` dan token `--font-heading` / `--font-sans` di `globals.css`. `MASTER.md:37-44` masih menyebut Lexend — abaikan, file itu placeholder |
...
Catatan tambahan: repo sudah `git init` dan punya riwayat commit. Identitas git masih placeholder (`autonomo-dev` / `dev@autonomo.id`, **repo-local saja**, bukan `--global`) — ganti ke nama & email asli sebelum ada remote, karena setelah di-push riwayat tidak bisa ditulis ulang dengan aman.
