 ...
 *
 * Tokens spent in the current view sit in the page head as a plain data pair
 * rather than a tinted pill — it is a figure to read, not a thing to press
 * (design.md § 4.3, § 5.4).
 */
        ...
        <div className="-mx-1 flex items-center gap-1.5 overflow-x-auto px-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {TABS.map((tab) => (
            <Link
              ...
              className={`inline-flex min-h-11 shrink-0 items-center gap-1.5 whitespace-nowrap rounded-md border px-3.5 text-[13px] font-medium transition-colors duration-200 ${
                active === tab.value
              ...
              return (
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-x-3 gap-y-2">
                    <div className="min-w-0">
                      <h2 className="truncate font-heading text-base font-semibold leading-snug text-foreground">
                        {row.customerName?.trim() || 'Customer'}
                      </h2>
                      <p className="tabular mt-0.5 flex items-center gap-1.5 text-sm text-foreground-muted">
                        <Phone width={13} height={13} aria-hidden="true" />+{row.customerPhone}
                      </p>
                    </div>
                  ...
                  <div className="mt-3.5 border-t border-dashed border-border pt-3.5">
                    ...
                    <dl className="mt-2.5 grid grid-cols-2 gap-x-4 gap-y-3 text-sm sm:grid-cols-[repeat(4,minmax(0,1fr))]">
